...
| platformInfo | byte[] | variable | Platform network connection information (see [appendix](#appendix-c-network-information) for more information) |
...
A ProRegTx or ProUpServTx is invalid if any of these conditions are true (applicable only when type is 1):
  ...
  1. platformInfo fails validation [(see rules)](dip-0003/network-info.md#plinfo_rules).
