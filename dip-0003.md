...
The ProRegTx might have the masternode network information field empty. This will result in the masternode being added to the masternode list in the PoSe-banned state and require the operator to issue a ProUpServTx. This is needed in case the owner wants to delegate the operator role to a hosting service without knowing the IP Address and port in advance.
...
| networkInfo | byte[] | variable | Masternode network connection information (see [appendix](#appendix-c-network-information) for more information) |
...
| platformInfo | byte[] | variable | Platform network connection information **(deprecated by extended network information format, see [appendix](#appendix-c-network-information) for more information)** |
...
A ProRegTx or ProUpServTx is invalid if any of these conditions are true (applicable only when type is 1):
  ...
  1. platformNodeID is null or is a non-hex string.
  2. platformNodeID is not unique in the network.
